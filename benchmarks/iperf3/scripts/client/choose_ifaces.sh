...
if [[ "$occurrences" -gt 1 ]]; then
    ...
    # CLIENT ONLY: take the top of $SERVER_IPS and use that type
    server_ip_type="$(echo ${SERVER_IPS[0]} | grep -oP "^[0-9]+.[0-9]+.")"
    else
        if [[ ! -z $ip17 && $server_ip_type == "1.17" ]]; then
            ip=$ip17
        elif [[ ! -z $ip18 && $server_ip_type == "1.18" ]]; then
            ip=$ip18
        elif [[ ! -z $ip10 && $server_ip_type == "10.7" ]]; then
            ip=$ip10
        else
            ip=$matching
        fi
